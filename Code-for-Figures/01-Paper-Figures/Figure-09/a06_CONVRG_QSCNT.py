...
for j in range(0, len(rho_p)):
    ...
    for ll in range(0, len(L_v)):
        ...
        #Ref_norm      = np.linalg.norm(Relaxing_particle.pos_vec, ord=2, axis=1)
...
plt.text(145, 1.5e-2, "$N^{-1}$", color='grey', fontsize=N_fs)
plt.plot(L_v[2:4], 5.*L_v[2:4]**(-2.0), '--', color='grey', linewidth=1.0)
plt.text(145, 4e-4, "$N^{-2}$", color='grey', fontsize=N_fs)
...
plt.savefig(save_plot_to + 'Figure_09a.pdf', format='pdf', dpi=500)

call(["pdfcrop", save_plot_to + 'Figure_09a.pdf', save_plot_to + 'Figure_09a.pdf'])
...
plt.savefig(save_plot_to + 'Figure_09b.pdf', format='pdf', dpi=500)

call(["pdfcrop", save_plot_to + 'Figure_09b.pdf', save_plot_to + 'Figure_09b.pdf'])
...
plt.savefig(save_plot_to + 'Figure_09c.pdf', format='pdf', dpi=500)

call(["pdfcrop", save_plot_to + 'Figure_09c.pdf', save_plot_to + 'Figure_09c.pdf'])
